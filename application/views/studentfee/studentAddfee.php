                        ...
                        <div class="table-responsive">
                            ...
                            <table class="table table-striped table-bordered table-hover example table-fixed-header">
                                ...
                                <tr>
                                    ...
                                    <th align="left"><?php echo $this->lang->line('credit_note'); ?></th>
...
//                                die;
                                foreach ($student_due_fee as $key => $fee) {
                                    $discount_percent = $fee->discount_amount;
                                    $discount_name  = $fee->discount_name;
                                    $discount_code = $fee->discount_code;
                                    ...
                                    foreach ($fee->fees as $fee_key => $fee_value) {
                                        ...
                                        if (!empty($fee_value->amount_detail)) {
                                            ...
                                            foreach ($fee_deposits as $fee_deposits_key => $fee_deposits_value) {
                                                ...
                                                if(trim(@$fee_deposits_value->status) != "deleted"){
                                                ...
                                                $amountPaidCheck = $amountPaidCheck + $fee_deposits_value->amount;
                                                }
                                            }
                                        }
                                        ...
                                        if($fee_value->type == "Tuition Fee" || trim(strtolower($fee_value->type ))== "saudi student"){
                                            $discount_unpaid = $balance_amount * $discount_percent  / 100;
                                        }
                                        ...
                                        if ($fee_value->due_date != "0000-00-00") 
                                            $dueData    =  date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($fee_value->due_date));
                                        $feeTax = 0;
                                        if(trim($fee_value->code) == "Previous Session Balance" && $previous_session_balance_tax["previous_session_balance_tax"] == "disabled")
                                            $feeTax = 0;
                                        else if($fee_value->is_taxable == "YES" )
                                        ...
                                        $payamentStatus = '<span class="label label-danger">'.$this->lang->line('unpaid').'</span>';
                                        if ($payment_amountStatus <= 0) 
                                            $payamentStatus = '<span class="label label-success">'.$this->lang->line('paid').'</span>';
                                        else if ($amountPaidCheck>0)
                                            $payamentStatus = '<span class="label label-warning">'.$this->lang->line('partial').'</span>';
                                        $display_discount = "";
                                        if($discount_unpaid>0)
                                            $display_discount =  "".$discount_code." - ".number_format($discount_unpaid, 2, '.', '');
                                       
                                        $display_none = "ss-none";
                                        $balanceAmount = ($tax_amount + $balance_amount - $discount_unpaid - $fee_paid);
                                        if($balanceAmount>0)
                                            $display_none = "";
                                        
                                        $payable_amount = ($tax_amount + $fee_value->amount - $discount_unpaid);
                                        $addTableClass= "dark-gray";
                                        if ($feetype_balance > 0 && strtotime($fee_value->due_date) < strtotime(date('Y-m-d'))) 
                                            $addTableClass= "danger font12";
                                        ?>
                                    <tr class="<?php echo $addTableClass?>">
                                        <td>
                                        ...
                                        <td align="left"><?php echo $fee_value->invoice_number; ?></td>
                                        <td align="left"><?php echo (trim($fee_value->creditnote_number) != '')?str_pad($fee_value->creditnote_number, 6, "0", STR_PAD_LEFT):''; ?></td>
                                        <td align="left"><?php echo $fee_value->name; ?></td>
                                        <td align="left"><?php echo $fee_value->code; ?></td>
                                        <td align="left" class="text text-left"><?php echo $dueData; ?></td>
                                        <td align="left" class="text text-left width85"><?php echo $payamentStatus; ?></td>
                                        <td class="text text-right"><?php echo $fee_value->amount; ?></td>
                                        <td class="text text-left"></td>
                                        <td class="text text-left"></td>
                                        <td class="text text-left"></td>
                                        <td class="text text-right"><?php echo $display_discount; ?></td>
                                        <td class="text text-right"><?php echo $tax_amount; ?></td>
                                        ...
                                        <td class="text text-right"><?php echo number_format($balanceAmount, 2, '.', ''); ?></td>
                                        <td class="text text-right"><?php echo number_format($balanceAmount, 2, '.', ''); ?></td>
                                        <td>
                                            <div class="btn-group pull-right">
                                                ...
                                                <button type="button"
                                                        ...
                                                        data-invoice_number="<?php echo $fee_value->invoice_number; ?>"
                                                        ...
                                                        data-calc_discount = "<?php echo $discount_unpaid; ?>"
                                                        data-discount_percentage = "<?php echo $student_discount_fee;?>"
                                                ...
                                                <button type="button"
                                                        ...
                                                        data-calc_discount = "<?php echo $discount_unpaid; ?>"
                                                        data-discount_percentage = "<?php echo $student_discount_fee;?>"
                                                        data-fee_paid = "<?php echo $payable_amount; ?>"
                                                        data-toggle="modal" data-target="#myCreditFeesModal"
                                                        class="btn btn-xs btn-default myCollectFeeBtn"
                                                        title="<?php echo $this->lang->line('add_fees'); ?>">
                                        ...
                                        if (!empty($fee_value->amount_detail)) {
                                            ...
                                            foreach ($fee_deposits as $fee_deposits_key => $fee_deposits_value) {
                                                ...
                                                ?>
                                                <tr class="white-td" <?php echo (trim(@$fee_deposits_value->status) == 'deleted') ?'style="text-decoration: line-through;"':''; ?>>
                                                    <td align="left"></td>
                                                    <td align="left"></td>
                                                    <td align="left"></td>
                                                    <td align="left"></td>
                                                    <td align="left"></td>
                                                    <td align="left"><a href="#" data-toggle="popover" class="detail_popover_log" > Logs</a>
                                                        <div class="fee_detail_popover_log" style="display: none">
                                                            <?php 
                                                            ...
                                                            if(isset($fee_deposits_value->log))
                                                                echo implode(" ",$fee_deposits_value->log);
                                                            ?>
                                                        </div></td>
...
<div class="modal fade" id="myCreditFeesModal" role="dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body pb0">
                <div class="form-horizontal">
                    <div class="box-body">
                        ...
                        <input  type="hidden" class="form-control" id="invoice_number" value="0" readonly="readonly"/>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input  id="date" name="admission_date" placeholder="" type="text" class="form-control"  value="<?php echo date($this->customlib->getSchoolDateFormat()); ?>" readonly="readonly"/>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="amount_error" class="col-sm-3 control-label"><?php echo $this->lang->line('amount'); ?> </label><small class="req"> *</small>
                            <div class="col-sm-9">
                        ...
                        <div class="form-group">
                            <label for="description" class="col-sm-3 control-label"><?php echo $this->lang->line('note'); ?></label>
            ...
            <div class="modal-footer">
                <div class="box-body">
                    ...
                    <button type="button" class="btn save_credit_note" id="load" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Processing"> <?php echo $currency_symbol; ?> <?php echo $this->lang->line('credit_notes'); ?> </button>
                </div>
            </div>
        </div>
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                ...
                                <label class="radio-inline">
                                    <input type="radio" name="payment_mode_fee" value="Credit Note"><?php echo $this->lang->line('credit_note'); ?>
                                </label>
                                <span class="text-danger" id="payment_mode_error"></span>
                            </div>
                        </div>
                        <div class="form-group" id="display_credit_notes" style="display:none;">
                            <label for="inputPassword3" class="col-sm-3 control-label"><?php echo $this->lang->line('credit_note'); ?></label>
                            <div class="col-sm-9">
                                <select name="pay_credit_notes" id="pay_credit_notes" class="form-control" onchange="managecreditnote();">
...
<script type="text/javascript">
    ...
    $(document).on('click', '.save_credit_note', function (e) {
        var $this = $(this);
        $this.button('loading');
        $this.removeClass('save_credit_note');
		//$this.hide();
        var form = $(this).attr('frm');
        var feetype = $('#feetype_').val();
        var date = $('#date').val();

        var amount = $('#myCreditFeesModal #amount').val(); // amount without calculation
        ...
        var amount_fine ="0";
        var description = $('#myCreditFeesModal #description').val();
        ...
        $.ajax({
            url: '<?php echo site_url("studentfee/addstudentfeecredit") ?>',
            type: 'post',
            data: {
                date: date, 
                type: feetype, 
    ...
    $(document).on('click', '.save_button', function (e) {
        ...
        var amount = $('#myFeesModal  #amount').val(); // amount without calculation
        var tax =  "0";// $('#amount_tax_included').val(); // calc tax amount
        var discount = "0";//$("#modal_discount").val(); // calc discount amount
        var amount_discount =  "0";//$("#modal_calc_discount").val();
        var amount_fine ="0";
        var description = $('#myFeesModal #description').val();
        var guardian_phone = $('#guardian_phone').val();
        var guardian_email = $('#guardian_email').val();
        var student_fees_master_id = $('#student_fees_master_id').val();
        var pay_credit_notes = $('#pay_credit_notes').val();
        var fee_groups_feetype_id = $('#fee_groups_feetype_id').val();
        var payment_mode = $('input[name="payment_mode_fee"]:checked').val();
        var student_fees_discount_id = "-1";
        $.ajax({
            ...
            data: {date: date, tax:tax,  type: feetype, amount: amount, amount_discount: amount_discount, 
                    amount_fine: amount_fine, description: description, 
                    pay_credit_notes:pay_credit_notes,
                    student_fees_master_id: student_fees_master_id, fee_groups_feetype_id: fee_groups_feetype_id, payment_mode: payment_mode, guardian_phone: guardian_phone, guardian_email: guardian_email, student_fees_discount_id: student_fees_discount_id},
            dataType: 'json',
            success: function (response) {
                $this.button('reset');
                if (response.status == "success") {
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("input[name='payment_mode_fee']").click(function(){
            $("#display_credit_notes").hide();
            var radioValue = $("input[name='payment_mode_fee']:checked").val();
            if(radioValue == "Credit Note"){
                var studentid = "<?php echo $id;?>";
                $.ajax({
                type: "post",
                url: '<?php echo site_url("studentfee/getcreditnotes") ?>',
                dataType: 'JSON',
                data: {'studentid': studentid},
                success: function (response) {
                    var len = response.length;
                    if(len<=0){
                        //$("input[name='payment_mode_fee']").val("Cash");
                        alert("You donot have valid credit note.");
                        $("input[name=payment_mode_fee][value='Cash']").prop("checked",true);
                    } else {
                        $("#display_credit_notes").show();
                        $("#pay_credit_notes").empty();
                        $("#pay_credit_notes").append("<option value=''>Select Credit Note</option>");
                        for( var i = 0; i<len; i++){
                            var id = response[i]['creditnote_number'];
                            var creditnote_display = id+" ("+response[i]['creditnote_admission_no']+" - "+(response[i]['creditnote_amount']-response[i]['creditnote_paid'])+")";
                            $("#pay_credit_notes").append("<option value='"+id+"' data-admissiono='"+response[i]['creditnote_admission_no']+"' data-amount='"+(response[i]['creditnote_amount']-response[i]['creditnote_paid'])+"'>"+creditnote_display+"</option>");
                        }
                    }
                }
            });
        ...
        $('.detail_popover_log').popover({
            ...
            html: true,
            content: function () {
                return $(this).closest('td').find('.fee_detail_popover_log').html();
            }
        });
    });
...
<script type="text/javascript">
        ...
        function managecreditnote(){
            var admissionno = $("#pay_credit_notes").find(':selected').attr('data-admissiono');
            var amount = $("#pay_credit_notes").find(':selected').attr('data-amount');
            var creditNumber = $("#pay_credit_notes").val();
            var payableamount = 0.00;
            if(parseFloat(amount) >= parseFloat($("#myFeesModal #amount").val()) ){
                payableamount = $("#myFeesModal #amount").val();
                
            } else {
                payableamount = parseFloat(amount);
            }
            $("#myFeesModal #amount").val(payableamount); 
           // alert(amount);
            $("#myFeesModal #description").val("credit note:"+creditNumber+" amount(SR):"+payableamount);
        }
...
<script type="text/javascript">
        $("#myCreditFeesModal").on('shown.bs.modal', function (e) {
        console.log("sdrere");
        e.stopPropagation();
        var data = $(e.relatedTarget).data();
        $("span[id$='_error']").html("");
        $('.fees_title').html("");
        $('#amount').val(data.fee_paid);
        var type = data.type;
        ...
        $("#final_amount").val(data.fee_paid)
        $('.fees_title').html("Credit Note for <b>" + group + ":</b> " + type );
    ...
    $("#myFeesModal").on('shown.bs.modal', function (e) {
        ...
        $("span[id$='_error']").html("");
        $('.fees_title').html("");
        $('#myFeesModal #amount').val(data.final_fee);
