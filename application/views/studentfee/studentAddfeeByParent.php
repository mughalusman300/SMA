                                        ...
                                        foreach ($student_due_fee as $key => $fee) {
                                            ...
                                            foreach ($fee->fees as $fee_key => $fee_value) {
                                                ...
                                                <td class="text text-right"><?php
                                                    echo$discount_code." ".(number_format($discount_unpaid, 2, '.', ''));
                                                    ?>
                                        ...
                                        foreach ($modal_student_due_fee as $key => $fee) {
                                            ...
                                            foreach ($fee->fees as $fee_key => $modal_fee_value) {
                                                ...
                                                if($modal_fee_value->type == "Tuition Fee" || $modal_fee_value->type == "Saudi Student"){
                                                    $modal_discount_unpaid = $modal_balance_amount * $discount_percent  / 100;
                                                }
                                                //$modal_total_discount_amount += $modal_discount_amount;
                                                //$modal_total_amount + $modal_balance_amount;
                                                $showRow = 0;
                                                ...
                                                if(floatval($showRow) > 0.01){
                                                    ...
                                                    <?php
                                                    if ($modal_fee_value->due_date == "0000-00-00") {

                                                    } else {

                                                        echo date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($modal_fee_value->due_date));
                                                    }
                                                    ?>
                                                </td>
                                                    <td> <?php
                                                    ...
                                                    <td class="text text-left"><?php
                                                        ...
                                                        if ($modal_balance_amount > 0) {
                                                            $display_none = "";
                                                            $modal_total_amount += $modal_fee_value->amount;
                                                    ...
                                                    <td class="text-left">
                                                        ...
                                                        if(trim($modal_fee_value->code) == "Previous Session Balance" && $previous_session_balance_tax["previous_session_balance_tax"] == "disabled"){
                                                            ...
                                                            $model_total_last +=(number_format(($modal_balance_amount - $modal_fee_paid- $modal_discount_unpaid), 2, '.', ''));
                                                            echo (number_format($modal_balance_amount - $modal_fee_paid- $modal_discount_unpaid, 2, '.', ''));
                                                        }  ?>
                                                        <?php
                                                        if(trim($modal_fee_value->code) == "Previous Session Balance" && $previous_session_balance_tax["previous_session_balance_tax"] == "disabled"){
                                                            ...
                                                            $amountPayAble =  (number_format(($modal_balance_amount - $modal_fee_paid -$modal_discount_unpaid), 2, '.', ''));
                                                        }
//<?php echo $amountPayAble? >
                                                        ?>
                                                        <input type="checkbox" name="enableAmountPayable[<?php echo $inputIndexCount; ?>]" data-index-dom="<?php echo $inputIndexCount; ?>" id="enableAmountPayable_" class="enableAmountPayable" data-amount_payable="<?php echo $amountPayAble?>" value="yes"> 
                                                    </td>

                                                    <td>

                                                        <input  data-index-dom="<?php echo $inputIndexCount; ?>" name="fee_group_session_id_print[]" class="fee_group_session_id_print" type="hidden" value="<?php echo $modal_fee_value->fee_session_group_id;  ?>" />
                                                        <input data-index-dom="<?php echo $inputIndexCount; ?>" type="hidden" name="modal_fee_fine[]" value="<?php echo $modal_fee_fine; ?>" class="modal_fee_fine"/>

                                                        <input data-index-dom="<?php echo $inputIndexCount; ?>" type="hidden" name="modal_fee_groups_feetype_id[]" value="<?php echo $modal_fee_value->fee_groups_feetype_id; ?>" class="modal_fee_groups_feetype_id"/>

                                                        <input data-index-dom="<?php echo $inputIndexCount; ?>" type="hidden" name="modal_fee_id[]" value="<?php echo $fee->id; ?>" class="modal_fee_id"/>
                                                        <input data-index-dom="<?php echo $inputIndexCount; ?>" type="hidden"  name="modal_student_id[]" class="modal_student_id" value="<?php echo $students[$modal_i]->id; ?>"/>
                                                        <input data-index-dom="<?php echo $inputIndexCount; ?>" type="hidden"  name="tax_to_pay[]" class="tax_to_pay" disabled value="<?php  
																	if($modal_fee_value->is_taxable == "YES")echo  (((float)$fee_tax["fee_tax"] * (float)$modal_balance_amount) / 100); else 0.00; ?>"/>

                                                        <input data-index-dom="<?php echo $inputIndexCount; ?>" type="hidden"  name="discount_percentage[]" class="discount_percentage" disabled value="<?php
                                                        if($modal_fee_value->type == "Tuition Fee"  || $modal_fee_value->type == "Saudi Student") {
                                                        ...
                                                        <input data-index-dom="<?php echo $inputIndexCount; ?>"  type="number"  id="amount_paying<?php echo $inputIndexCount; ?>" name="amount_paying[<?php echo $inputIndexCount; ?>]" onkeyup="updateTotal(<?php echo ($modal_fee_value->is_taxable== 'YES') ?  $fee_tax["fee_tax"]:0; ?>)" class="amount_paying"  data-amount_payable="<?php echo $amountPayAble?>" value="0" disabled="disabled"/>
                                                    </td> 
                                                   </tr>
                                                <?php 
                                                $inputIndexCount++;
...
<tfoot>
                                    <tr class="box box-solid total-bg">
                                        ...
                                        <td><?php echo number_format($modal_total_amount,2);?></td>
                                        <td></td>
                                        <td></td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $(document).on('click', '.generate_invoices', function () {
            ...
            if (array_to_print.length == 0) {
                ...
                $.ajax({
                    url: '<?php echo site_url("studentfee/generateInvoicesManually") ?>',
                    type: 'post',
                    data: {'data': JSON.stringify(array_to_print)},
                    success: function (response) {
    ...
    $(function () {
      ...
      //  $(this).onkeyup();
      $(document).on('change', ".enableAmountPayable", function (e) {
            var domindex = $(this).data('index-dom');
            var amount_payable = $(this).data('amount_payable');
            var amount = document.getElementById('amount_paying'+domindex);
            
        if($(this).prop('checked')== true){  
            amount.disabled = false;
            amount.value = amount_payable;
