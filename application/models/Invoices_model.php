...
class Invoices_model extends CI_Model {
    ...
    public function update_invoices_number($id, $data) {
     ...
     public function getInvoiceTypes() {
        $this->db->select('invoices.inv_type')->from('invoices')->group_by("invoices.inv_type");
        $queryRes = $this->db->get();
        return $queryRes->result_array();
    }
    
    public function getInvoiceStatus() {
        $this->db->select('invoices.status')->from('invoices')->group_by("invoices.status");
        $queryRes = $this->db->get();
        return $queryRes->result_array();
    }
    /**
    ...
    public function get($id = null, $guardian_id = null) {
       ...
       // $this->db->join('(select students.parent_id,students.guardian_id  from students group by parent_id) as students', ' invoices.parent_id = students.parent_id1');
	   $this->db->join('students', ' `invoices`.`admission_no` = `students`.`admission_no`');
        if ($guardian_id != '')
    ...
    public function getInvoicesList($id = null, $guardian_id = null,$admission_no = null,$invoice_no = null, $getTotal = false,$date_from=null, $date_to = null,$status=null,$invoice_type=null, $limit = 10, $start = 0, $export=false) {

        $this->db->select("invoices.*, students.parent_id,students.guardian_id , ( CASE WHEN credit_note.invoice_number IS NULL THEN 0 ELSE credit_note.invoice_number END) as credited")->from('invoices');
        $this->db->join('students', ' `invoices`.`admission_no` = `students`.`admission_no`');
        $this->db->join('credit_note', ' `invoices`.`id` = `credit_note`.`invoice_number`','left');
        if ($guardian_id != '')
            $this->db->where('students.guardian_id', $guardian_id);
        if ($id != null) {
            $this->db->where('invoices.id', $id);
         ...
         if ($admission_no != null) {
            $this->db->where('invoices.admission_no', $admission_no);
        }
         if ($invoice_no != null) {
            $this->db->where('invoices.invoice_number', $invoice_no);
        }
		if ($date_from != null) {
            $this->db->where('invoices.invoice_date>=', $date_from);
        }
		if ($date_to != null) {
            $this->db->where('invoices.invoice_date<=', $date_to);
        }
         ...
         if ($invoice_type != null) {
            $this->db->where('invoices.inv_type', $invoice_type);
        }
     ...
     * @return mixed
     */
    public function getInvoiceDetails($invoiceId = null, $guardian_id = null,$admission_no = null,$invoice_no = null, $getTotal = false,$date_from=null, $date_to = null) {
		...
		if ($guardian_id != ''){
            $this->db->join('students as students_parent', ' invoices.parent_id = students_parent.parent_id');
       ...
       //  die;
        ...
        foreach ($query as $i => $invoices) {
            ...
            $this->db->where('admission_no', $invoices['admission_no']);
        ...
        public function getInvoiceBookDetails($invoiceId = null, $guardian_id = null,$admission_no = null,$invoice_no = null, $getTotal = false,$date_from=null, $date_to = null) {

        $this->db->select(
                "invoices.*,"
                . "invoices_details.book_order_id,"
                . "invoices_details.book_id,"
                . "invoices_details.amount,"
                . "invoices_details.total_amount,"
                . "invoices_details.date_created"
                //      . "parents.*,"
       ...
       //  die;
        ...
        foreach ($query as $i => $invoices) {
            ...
            $sql = 'select 
                            store_books.*, 
                        book_store_orders.*, store_books.book_id as book_no  FROM store_books 
                        INNER JOIN 
                            book_store_orders ON store_books.id = book_store_orders.book_id
                        INNER JOIN 
    ...
    public function checkOrderInvoices($orderid) {

        $this->db->select(' * ')->from('invoices_details');
        $this->db->join('invoices', 'invoices.id = invoices_details.invoice_id');
        $this->db->where('book_order_id', $orderid);
        $this->db->where("invoices.status <> 'deleted'");
        $query = $this->db->get();
        return $query->result_array();
    }
...
//die;e;
                ...
                $html[]=  $count . " -- " . $student_id . " recepit generated successfully <br>";
                //if($count>5000)
    ...
    public function setStudentFeeDetails($fee_groups_feetype_id, $fee_master_id, $fee_session_group_id) {
        ...
        $sql = "SELECT 
                        ...
                        FROM
                            student_session
                                INNER JOIN
                            students ON students.id = student_session.student_id AND students.is_active = 'yes'
                                INNER JOIN
                            student_fees_master ON student_session.id = student_fees_master.student_session_id
                                INNER JOIN
                            classes ON classes.id = student_session.class_id
