...
class Examschedule_model extends CI_Model {
    ...
    public function copyExamToAllSection($class_id, $section_id, $exam_id) {
        $queryToCopyDataFromExam = "INSERT INTO `exam_schedules`
(session_id,
`exam_id`,
`teacher_subject_id`,
`date_of_exam`,
...
`note`,
`is_active`) SELECT 
exam_schedules.`session_id`,
exam_schedules.`exam_id`,
tts.id,
exam_schedules.`date_of_exam`,
exam_schedules.`start_to`,
exam_schedules.`end_from`,
...
FROM
    exam_schedules
        INNER JOIN
    teacher_subjects ON exam_schedules.teacher_subject_id = teacher_subjects.id
        INNER JOIN
    (SELECT 
        *
    FROM
    ...
    WHERE
        class_id = " . $this->db->escape($class_id) . "
            AND teacher_subjects.id NOT IN (SELECT 
                teacher_subject_id
            FROM
                exam_schedules)) tts ON teacher_subjects.subject_id = tts.subject_id
        ...
        return $query = $this->db->query($queryToCopyDataFromExam);
    }
