...
class Studentfee_model extends CI_Model {
    ...
    public function remove($id, $sub_invoice) {
        ...
        if ($q->num_rows() > 0) {
            ...
            $a[$sub_invoice]["status"] = 'deleted';
            $a[$sub_invoice]["log"][] = '<p class="text text-info"> Reverted By: '.$result["name"].", At: ".date("d-m-y h:i:s")."</p>";
            if (!empty($a)) {
                $data['amount_detail'] = json_encode($a);
                $this->db->where('id', $id);
                $this->db->update('student_fees_deposite', $data);
                if(isset($a[$sub_invoice]["creditnote"])){
                    $this->db->query("UPDATE credit_note SET credit_paid = (credit_paid -".$a[$sub_invoice]["amount"]."), credit_balance=(creditnote_amount-credit_paid) WHERE id = ".$a[$sub_invoice]["creditnote"]);
                }
            } else {
                $this->db->where('id', $id);
        ...
        $query = "SELECT "
                ...
                . ", fees_discounts.name AS discount_name,
                       ...
                       INNER JOIN student_fees_master on student_fees_master.student_session_id=student_session.id 
                                and student_fees_master.fee_session_group_id=" . $this->db->escape($feegroup_id) . " "
                . "LEFT JOIN student_fees_deposite on student_fees_deposite.student_fees_master_id=student_fees_master.id and student_fees_deposite.fee_groups_feetype_id=" . $this->db->escape($fee_groups_feetype_id) . "  "
                . "INNER JOIN fee_groups_feetype on fee_groups_feetype.id = " . $this->db->escape($fee_groups_feetype_id) . " "
                . "LEFT JOIN fees_discounts ON fees_discounts.id = student_fees_master.fee_discount_id  "
                . "WHERE `student_session`.`session_id` = " . $this->current_session . " AND 
