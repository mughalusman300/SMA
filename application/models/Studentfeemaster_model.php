...
class Studentfeemaster_model extends CI_Model {
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null) {
        $sql = "SELECT IFNULL(`student_fees_master`.`id`, '0') as `student_fees_master_id`,`classes`.`id` AS `class_id`,"
                ...
                . " `students`.`gender`, (SELECT discount_ids FROM student_indiviual_discount WHERE student_admission_no = students.admission_no COLLATE utf8_unicode_ci order by id  desc limit 1) as discount_id  "
    ...
    public function searchStudentsFeeByClassSection($class_id = null, $section_id = null, $admission_no = null,$discountentry = false) {
        $sql = "SELECT IFNULL(`student_fees_master`.`id`, '0') as `student_fees_master_id`,`classes`.`id` AS `class_id`,"
                . " `student_session`.`id` as `student_session_id`, `students`.`id`, "
                . "`classes`.`class`, `sections`.`id` AS `section_id`, `sections`.`section`, "
                ...
                . " `students`.`gender`,
                    ...
                    fees_discounts.name AS discount_name,
                ...
                . "LEFT JOIN fees_discounts ON fees_discounts.id = student_fees_master.fee_discount_id";
        ...
        if($discountentry)
            $sql .= " AND student_fees_master.fee_discount_id > 0 ";
    ...
    public function delete($fee_session_groups, $array) {
        $masterId = $this->checkDataFordelete($fee_session_groups, $array);
        $invoicenumbers = $this->getInvoicesDelete($fee_session_groups, $array);
        $this->db->where('fee_session_group_id', $fee_session_groups);
        $this->db->where_in('student_session_id', $array);
        if(!empty($masterId["master_id"]))
            $this->db->where_not_in('id', $masterId["master_id"]);
        $this->db->delete('student_fees_master');
        
        if(!empty($invoicenumbers))
        {
            $this->db->where_in('invoice_number', $invoicenumbers);
            $this->db->update('student_fees_master',['invoice_number'=>null, 'is_invoiced'=>null]);
            
            $this->db->where_in('id', $invoicenumbers);
            $this->db->update('invoices',['status'=>'deleted']);
        }
        
    }
    
    public function getInvoicesDelete($fee_session_groups, $array) {
        $masterId = $this->checkDataFordelete($fee_session_groups, $array);
        $this->db->where('fee_session_group_id', $fee_session_groups);
        $this->db->where_in('student_session_id', $array);
        if(!empty($masterId["master_id"]))
        ...
        if (!empty($result)) {
            foreach($result as $key=>$value){
                if(trim( $value->invoice_number)!= '')
                    $outputArray[] = $value->invoice_number;
            }
        }
       return $outputArray; 
    }
    
    public function checkDataFordelete($fee_session_groups, $array) {
        $sql = "SELECT 
                    student_fees_master.id,
                    students.admission_no,
                    students.firstname, student_fees_master.invoice_number, student_fees_deposite.amount_detail     
                FROM
                  student_session
                      INNER JOIN
                  ...
                  student_fees_master ON student_session.id = student_fees_master.student_session_id
                      INNER JOIN
                  student_fees_deposite ON student_fees_master.id = student_fees_deposite.student_fees_master_id
                      INNER JOIN
                  fee_groups_feetype ON fee_groups_feetype.id = student_fees_deposite.fee_groups_feetype_id
                WHERE
       
           fee_groups_feetype.fee_session_group_id = ".$fee_session_groups." and student_session_id in ( ". implode(",", $array).")
       ";
        $query = $this->db->query($sql);
        $outputArray = ["admission_no"=>[],"master_id"=>[],"student_name"=>[],"invoice_number"=>[]];
        $result = $query->result();
        $return_array = array();
        if (!empty($result)) {
            foreach($result as $key=>$value){
                $amountDetails = json_decode($value->amount_detail);
                $unassignCheck = false;
                $countDepositDelete = 0;
                $countDeposit = count($amountDetails);
                foreach($amountDetails as $amountItems){
                    if(isset($amountItems->status) && trim($amountItems->status) == "deleted"){
                        $countDepositDelete = $countDepositDelete + 1;
                    }
                }
                if($countDeposit > $countDepositDelete ){
                    $outputArray["admission_no"][] = $value->admission_no;
                    $outputArray["student_name"][] = $value->firstname;
                    $outputArray["master_id"][] = $value->id;
                    if(trim($value->invoice_number)!= '')
                        $outputArray["invoice_number"][] = $value->invoice_number;
                }
            }
        }
    ...
    public function getStudentFeesByStudentId($student_id) {
    ...
    sections ON sections.id = student_session.section_id
        INNER JOIN
    fee_session_groups ON student_fees_master.fee_session_group_id = fee_session_groups.id
        INNER JOIN
    fee_groups ON fee_groups.id = fee_session_groups.fee_groups_id
        LEFT JOIN
    fees_discounts ON fees_discounts.id = student_fees_master.fee_discount_id
WHERE
    students.id = " . $student_id . " ORDER BY `student_fees_master`.`id`";
    ...
    public function fee_creditnote($data,  $invoice_number=0, $student_fees_master_id=0) {
            ...
            $dataInvoiceData = $data;
            $dataInvoiceData["creditnote_id"] = $invoice_number;
            $this->db->insert('creditnote_details', $dataInvoiceData);
            $inserted_id = $this->db->insert_id();
            if ($student_fees_master_id > 0) {
                $this->db->where('id', $student_fees_master_id);
                $this->db->update('student_fees_master', array('creditnote_number' =>$invoice_number));
            }
    ...
    public function getFeeBetweenDate($start_date, $end_date,$getTotal = false,$limit = 10, $start = 0, $export=false) {
        ...
        $this->db->select('`student_fees_deposite`.*,students.admission_no,students.firstname,'
                ...
                . '`feetype`.`code`,student_fees_master.student_session_id, fees_discounts.name as discount_name, 
                fees_discounts.code as discount_code, 
                fees_discounts.amount as discount_amount')->from('student_fees_deposite');
        ...
        $this->db->join('fees_discounts', 'fees_discounts.id = student_fees_master.fee_discount_id','LEFT');
        $st_date = strtotime($start_date);
            $ed_date = strtotime($end_date);
        for ($i = $st_date; $i <= $ed_date; $i += 86400) {
            
            $find = date('Y-m-d', $i);
            $this->db->or_like('student_fees_deposite.amount_detail', $find);
        }
            ...
            foreach ($result_value as $key => $value) {
                ...
                if (!empty($return)) {
                    foreach ($return as $r_key => $r_value) {
                        if(trim(@$r_value->status) == "deleted")
                        continue;
                        $a['id'] = $value->id;
                      ...
                      //      $discount = (($r_value->amount*$discount_amount)/100);
                      //  else
                      //      $discount = 0;
                        
                        $a["tax"] = $r_value->tax;//(($fee_tax["fee_tax"] * ($r_value->amount - $discount))/100);//
                        $a['date'] = $r_value->date;
                        $a['amount_discount'] = $r_value->amount_discount;//(($r_value->amount*$discount_amount)/100);//
