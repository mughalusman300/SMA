...
class Staff_model extends CI_Model {
    ...
    public function getDataForUpdatePassword($id = null) {



        $this->db->select('staff.*,roles.name as user_type,roles.id as role_id')
                ->from('staff')
                ->join("staff_roles", "staff_roles.staff_id = staff.id", "left")
                ->join("roles", "staff_roles.role_id = roles.id", "left");

$this->db->where("trim(staff.pass_key) <> '' ");
    ...
    public function getStaff($employee_id) {
    ...
    public function updatestaff($data, $roles = array(), $leave_array = array()) {
        ...
        $this->db->update_batch('staff_roles', array($roles),"staff_id");

        foreach ($leave_array as $key => $value) {
            $leave_array[$key]['staff_id'] = $staff_id;
        }
        if(count($leave_array) > 0){
            $this->db->update_batch('staff_leave_details', $leave_array);
        }
    ...
    function getClassTeacher($role, $active = 1,$student_id) {
		
			
        
		$this->db->select("staff.*,staff_designation.designation,department.department_name as department,roles.name as user_type")
		->join('staff_designation', "staff_designation.id = staff.designation", "left")
		->join('staff_roles', "staff_roles.staff_id = staff.id", "left")
		->join('roles', "roles.id = staff_roles.role_id", "left")
		->join('department', "department.id = staff.department", "left")
		->join('class_teacher', "class_teacher.staff_id = staff.id", "inner");
		$this->db->where("staff.is_active", $active);
		$this->db->where("roles.name", $role);
		if(is_array($student_id)){
			$this->db->where("class_teacher.class_id IN (SELECT class_id FROM student_session WHERE student_id in ('".implode("','",$student_id)."'))");
			$this->db->where("class_teacher.section_id IN (SELECT section_id FROM student_session WHERE student_id in ('".implode("','",$student_id)."'))");
		}else{
			$this->db->where("class_teacher.class_id IN (SELECT class_id FROM student_session WHERE student_id = ".$student_id.")");
			$this->db->where("class_teacher.section_id IN (SELECT section_id FROM student_session WHERE student_id = ".$student_id.")");
		}
    ...
    function getStaffbyrole($id) {
        ...
        $this->db->or_where("staff_roles.role_id", 16);
        $this->db->or_where("staff_roles.role_id", 19);
