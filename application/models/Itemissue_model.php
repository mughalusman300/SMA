...
class Itemissue_model extends CI_Model {
    ...
    public function get($id = null) {
        ...
        /*$sql = "SELECT item_issue.*,item.name as `item_name`,item.item_category_id,item_category.item_category ,staff.employee_id,staff.name ,staff.surname,roles.name FROM `item_issue` INNER JOIN item on item.id=item_issue.item_id INNER JOIN item_category on item_category.id=item.item_category_id INNER JOIN staff on staff.id=item_issue.issue_to INNER JOIN staff_roles on staff_roles.staff_id =staff.id INNER JOIN roles on roles.id= staff_roles.role_id";*/
         $this->db->select("item_issue.*,item.name as `item_name`,item.item_category_id,item_category.item_category ,staff.employee_id,staff.name ,staff.surname,roles.name")->from('item_issue');
         $this->db->join('item', 'item.id=item_issue.item_id');
         $this->db->join('item_category', 'item_category.id=item.item_category_id');
        //Note: Error here becuase instead of staff.id there is name of the staff in item_issue table. During issue item there should be save id of staff not name of staff:
         $this->db->join('staff', 'staff.id=item_issue.issue_to');
