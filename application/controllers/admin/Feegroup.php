...
class FeeGroup extends Admin_Controller {
    ...
    function index() {
        ...
        $feegroup_result = $this->feegroup_model->getlist();
    ...
    function delete($id) {
        ...
        if(!$this->feegroup_model->check_group($id)){
            $this->feegroup_model->remove($id);
                $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Fee group deleted successfully.</div>');
        }else{
            $this->session->set_flashdata('msg', '<div class="alert alert-danger text-left">Fee group cannot be remove, as it already assigned amount in fee master.</div>');
        }
        redirect('admin/feegroup/index');
    }
    function active($id) {
