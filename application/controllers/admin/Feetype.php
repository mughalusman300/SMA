...
class Feetype extends Admin_Controller {
    ...
    function index() {
        ...
        $feegroup_result = $this->feetype_model->getlist();
    ...
    function delete($id) {
        ...
        if(!$this->feetype_model->check($id)){
            $this->feetype_model->remove($id);
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Fee type deleted successfully.</div>');
        }else{
            $this->session->set_flashdata('msg', '<div class="alert alert-danger text-left">Fee type cannot be remove, as it already assigned amount in fee master.</div>');
        }
        redirect('admin/feetype/index');
    }
