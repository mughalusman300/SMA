                ...
                if (!empty($result)) {
                    for ($i = 1; $i <= count($result); $i++) {
                        ...
                        if($employee_data['password'] != ""){
                            $data_insert['pass_key'] = $password;
                            $data_insert['password'] = $employee_data['password']; }
                        if($employee_data['employee_id'] != "")
                            $data_insert['employee_id'] = $employee_data['employee_id'];
                        if($employee_data['department'] != "")
                            $data_insert['department'] = $employee_data['department'];
                        if($employee_data['designation'] != "")
                            $data_insert['designation'] = $employee_data['designation'];
                        if($employee_data['qualification'] != "")
                            $data_insert['qualification'] = $employee_data['qualification'];
                        if($employee_data['work_exp'] != "")
                            $data_insert['work_exp'] = $employee_data['work_exp'];
                        if($employee_data['name'] != "")
                            $data_insert['name'] = $employee_data['name'];
                        if($employee_data['contactno'] != "")
                            $data_insert['contact_no'] = $employee_data['contactno'];
                        if($employee_data['emergency_no'] != "")
                            $data_insert['emergency_contact_no'] = $employee_data['emergency_no'];
                        if($employee_data['email'] != "")
                            $data_insert['email'] = $employee_data['email'];
                        if($employee_data['dob'] != "")
                            $data_insert['dob'] = $employee_data['dob'];//date('Y-m-d', $this->customlib->datetostrtotime($employee_data['dob'])),
                        if($employee_data['marital_status'] != "")
                            $data_insert['marital_status'] = $employee_data['marital_status'];
                        if($employee_data['date_of_joining'] != "")
                            $data_insert['date_of_joining'] = $employee_data['date_of_joining'];//date('Y-m-d', $this->customlib->datetostrtotime($employee_data['date_of_joining'])),
                        if($employee_data['date_of_leaving'] != "")
                            $data_insert['date_of_leaving'] = '';
                        if($employee_data['address'] != "")
                            $data_insert['local_address'] = $employee_data['address'];
                        if($employee_data['permanent_address'] != "")
                            $data_insert['permanent_address'] = $employee_data['permanent_address'];
                        if($employee_data['note'] != "")
                            $data_insert['note'] = $employee_data['note'];
                        if($employee_data['surname'] != "")
                            $data_insert['surname'] = $employee_data['surname'];
                        if($employee_data['mother_name'] != "")
                            $data_insert['mother_name'] = $employee_data['mother_name'];
                        if($employee_data['father_name'] != "")
                            $data_insert['father_name'] = $employee_data['father_name'];
                        if($employee_data['gender'] != "")
                            $data_insert['gender'] = $employee_data['gender'];
                        if($employee_data['account_title'] != "")
                            $data_insert['account_title'] = $employee_data['account_title'];
                        if($employee_data['bank_account_no'] != "")
                            $data_insert['bank_account_no'] = $employee_data['bank_account_no'];
                        if($employee_data['bank_name'] != "")
                            $data_insert['bank_name'] = $employee_data['bank_name'];
                        if($employee_data['ifsc_code'] != "")
                            $data_insert['ifsc_code'] = $employee_data['ifsc_code'];
                        if($employee_data['bank_branch'] != "")
                            $data_insert['bank_branch'] = $employee_data['bank_branch'];
                            //'payscale' => $employee_data['payscale'],
                        if($employee_data['basic_salary'] != "")
                            $data_insert['basic_salary'] = $employee_data['basic_salary'];
                            //'epf_no' => $employee_data['epf_no'],
                        if($employee_data['contract_type'] != "")
                            $data_insert['contract_type'] = $employee_data['contract_type'];
                        if($employee_data['shift'] != "")
                            $data_insert['shift'] = $employee_data['shift'];
                        if($employee_data['location'] != "")
                            $data_insert['location'] = $employee_data['location'];
                        if($employee_data['facebook'] != "")
                            $data_insert['facebook'] = $employee_data['facebook'];
                        if($employee_data['twitter'] != "")
                            $data_insert['twitter'] = $employee_data['twitter'];
                        if($employee_data['linkedin'] != "")
                            $data_insert['linkedin'] = $employee_data['linkedin'];
                        if($employee_data['instagram'] != "")
                            $data_insert['instagram'] = $employee_data['instagram'];
                        if($employee_data['is_active'] != "")
                            $data_insert['is_active'] = $employee_data['is_active'];
                        ...
                        $checkStaffExist = false;
                        $staffData = $this->staff_model->getStaff($data_insert["employee_id"]);
//                        echo "<pre>";
//                        print_r($staffData);
//                        die;
                        if(count($staffData)>0){
                            $data_insert["id"] =   $staffData[0]["id"]; 
                            $this->staff_model->updatestaff($data_insert, $role_array, $leave_array);
